...
/*
    ALL IMPLEMENTATION COMMENTS WRITTEN AS UPPERCASE. ALL CHANGES ABOUT "ENQUEUE PROCESS BY ITS BURST TIME OR PRIORITY VALUES".

    PREEMPTIVE ÖZELLİĞİ SCHEDULING METOTLARINDA I/O BURST'E SAHİP İŞLEMLERİN KURALLAR ÇERÇEVESİNDE KUYRUKLAR ARASINDA TAŞINMASI İLE ELDE EDİLİYOR.

    3 METOTTA AYNI KODLARIN TEKRARI GÖRÜLMEKTEDİR. SADECE İLGİLİ METOTLARIN İMPLEMENTASYONU YAPILACAĞINDAN BU YOL İZLENMİŞTİR.
...
void sjf_scheduler()
{
    ...
    while(job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter) //check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY; //set flag to ready
            dequeue(job_queue, p->pid); //remove from job queue
            enqueue_orderby_burst_time(ready_queue, p); //MOVE PROCESS FROM JOB QUEUE TO READY QUEUE ORDER BY ITS BURST TIME VALUE. 
        }
        else
        {
            break; //no more process has arrived yet
    ...
    while (current != NULL) //loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_burst_time(ready_queue, p); //MOVE PROCESS FROM WAITING QUEUE TO READY QUEUE ORDER BY ITS BURST TIME VALUE.   
            }
        }
    }
...
void priority_scheduler()
{
    ...
    while(job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter) //check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY; //set flag to ready
            dequeue(job_queue, p->pid); //remove from job queue
            enqueue_orderby_priority(ready_queue, p); //MOVE PROCESS FROM JOB QUEUE TO READY QUEUE ORDER BY ITS PRIORITY VALUE. 
        }
        else
        {
            break; //no more process has arrived yet
    ...
    while (current != NULL) //loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_priority(ready_queue, p); //MOVE PROCESS FROM WAITING QUEUE TO READY QUEUE ORDER BY ITS PRIORITY VALUE. 
            }
        }
    }
